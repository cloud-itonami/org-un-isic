...
;;
;; THE TWO CHECKS THIS REGISTER USES ARE NOT INTERCHANGEABLE.
;;
...
;;
;; The e-Gov law branch is kept from the proven ministry registers and its
;; self-test still runs, because a branch that nothing exercises is a branch
;; that can rot silently -- but this register cites no statute, so no entry
;; uses it.
;;
;; THE HOSTS HERE ANSWER 404, NOT 403, FOR A PATH THAT DOES NOT EXIST.
;; Measured 2026-09-01: unstats.un.org serves a small IIS not-found page
;; ("404 - File or directory not found."); ec.europa.eu serves a large
;; Eurostat-branded one ("Status 404 - Eurostat"). Both at the requested URL.
;; 404 is not 2xx so status still discriminates, but a checker expecting the
;; go.jp hosts' 403 refusal would misreport two hosts that answered clearly.
;; The status each host actually returned is pinned in :host/missing-status
;; and re-measured on every run. Neither host has served a bot challenge to a
;; paced client; the AWS WAF challenge detector is kept anyway, because a
;; detector removed because the threat has not appeared yet is a detector that
;; will not be there when it does.
;;
...
;; body rather than against the live WAF, because whether a WAF is engaged at
;; any moment is not under this run's control.
;;
...
;;
;; FETCHES ARE SERIAL AND PACED, AND THAT IS LOAD-BEARING PRACTICE, NOT STYLE.
;; The ministry registers measured that concurrent fetches are what trip their
;; hosts' bot challenge. Neither host here has been seen to challenge a paced
;; client, and no burst behaviour is claimed for either; fetching serially with
;; a pace is what makes that claim cheap to keep instead of expensive to learn.
;;
;; WHY fetch AND NOT curl. The ministry registers measured go.jp hosts
;; blocking on the TLS fingerprint. Neither host here was measured to do
;; that, but the verifier reads bytes and decodes charset itself, which curl
;; would make awkward; fetch is kept for parity with the proven sibling.
